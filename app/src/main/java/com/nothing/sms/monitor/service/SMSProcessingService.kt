...
class SMSProcessingService : Service() {
     ...
     * 推送短信内容到所有已启用的推送服务
     */
    private suspend fun pushSMSToServices(smsId: Long, sender: String, body: String, timestamp: Long) {
        val enabledServices = pushServiceManager.getEnabledServices()
        ...
        serviceScope.launch {
            try {
                ...
                if (record.retryCount >= MAX_RETRY_COUNT) {
                    Timber.w("推送记录 #${record.id} 已达到最大重试次数 ${record.retryCount}")
                ...
                result.fold(
                    ...
                    onFailure = { e ->
                        // 判断是否是最后一次重试
                        val finalStatus = if (currentRetry >= MAX_RETRY_COUNT) {
                        ...
                        Timber.e(e, "重试推送到 ${service.serviceName} 失败，短信ID: ${record.smsId}")
    ...
    private fun markMaxRetryRecordsAsFailed() {
        serviceScope.launch {
            try {
                ...
                val pendingWithMaxRetries = smsDatabase.getPendingRecordsWithMaxRetry(MAX_RETRY_COUNT)
        ...
        serviceScope.launch {
            try {
                ...
                for (sms in pendingMessages) {
                    ...
                    // 检查是否所有服务都已经有记录
                    val allServicesHaveRecords = enabledServices.all { it.serviceType in existingServiceTypes }
                    ...
                    if (allServicesHaveRecords) {
                        ...
                        val failedRecords = smsDatabase.getPushRecordsBySmsId(sms.id)
                            .filter { it.status == SMSDatabase.STATUS_FAILED && it.retryCount < MAX_RETRY_COUNT }
                    ...
                    // 筛选未创建推送记录的服务
                    val servicesToPush = enabledServices.filter { it.serviceType !in existingServiceTypes }
